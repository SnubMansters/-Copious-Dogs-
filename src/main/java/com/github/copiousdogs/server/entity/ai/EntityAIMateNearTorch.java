...
public class EntityAIMateNearTorch extends EntityAIBase
{
	...
	private World world;
	private EntityDogServer mate;
	private float radius;
	
	int spawnBabyDelay;
	double speed;
	
	public EntityAIMateNearTorch(EntityDogServer dog, double speed, float radius)
	{
		this.dog = dog;
	...
	private EntityDogServer getNearbyMate()
	{
		double d = 15;
		List list = world.getEntitiesWithinAABB(dog.getClass(), dog.boundingBox.expand(d, d, d));
		double d0 = Double.MAX_VALUE;
		EntityDogServer animal = null;
		Iterator iterator = list.iterator();
		
		while (iterator.hasNext())
		{
			EntityDogServer animal1 = (EntityDogServer)iterator.next();
